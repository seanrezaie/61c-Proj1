...
char *read_line(FILE *fp) {
  ...
  while (1) {
      ...
      if (!fgets(buffer + currPos, bufferSize - currPos, fp)) {
          if (currPos == 0) {
              free(buffer);
              return NULL;
          }
  ...
  char *finalBuffer = realloc(buffer, currPos + 1);
  return finalBuffer ? finalBuffer : buffer;
}
